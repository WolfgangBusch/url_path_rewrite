...
#     rewrite($params)                 [identisch zu url_simple_rewrite]
#        is_normurl()                  [identisch zu url_simple_rewrite]
#        set_url($article)
#           get_newest_dirname($article)
#           proof_name($proofstr,$param,$article)
#              unique_url($article,$metainfo,$value)
#              rand_string($nz)
#              allowed_name($name)     (class url_path_config)
#              allowed_chars()         (class url_path_config)
#           mode_url_clang($url,$clang_id)
#           sql_action($sql,$query)    (class url_path_config)
#        get_url($article)
#           mode_url_clang($url,$clang_id)
#     set_current()                    [identisch zu url_simple_rewrite]
#        is_normurl()                  [identisch zu url_simple_rewrite]
#        get_current_clangid($req_url)
#           get_current_clang($req_url,$clang_val)
...
public static function set_url($article) {
   #   Rueckgabe des Custom URLs eines Artikels in der konfigurierten Form ohne
   #   fuehrenden '/' im Backend-Fall. Alle dafuer benoetigten Artikel-Parameter
   #   werden aus dem Artikel-Objekt genommen. Der URL wird als Artikel-MetaInfo
   #   (REWRITER_URL) in der Tabelle rex_article abgelegt. Anschliessend wird
   #   der Artikel-Cache geloescht.
   ...
   #      url_path_config::sql_action($sql,$query)
   #
   ...
   url_path_config::sql_action($sql,$quu);
   url_path_config::sql_action($sql,$qub);
   if($par_id==$art_id) url_path_config::sql_action($sql,$qud);
   #
...
public static function get_url($article) {
   #   Rueckgabe des Custom URLs eines Artikels (ohne fuehrenden '/')
   #   im Frontend-Fall. Er wird als Wert der Spalte REWRITER_URL aus der
   #   Tabelle rex_article ausgelesen.
   #   $article           Artikel-Objekt
   #   benutzte functions:
   #      self::mode_url_clang($url,$clang_id)
   #
...
public static function mode_url_clang($url,$clang_id) {
   ...
   #   Sprachkennzeichnung ergaenzt ist.
...
public static function proof_name($proofstr,$param,$article) {
   ...
   #      self::rand_string($nrand)
   #      url_path_config::allowed_name($name)
   #      url_path_config::allowed_chars()
   #
   $nrand=12;   // fester Parameter fuer self::rand_string($nrand)
   $art_id=$article->getId();
   ...
   if(!url_path_config::allowed_name($string)):
     ...
     if(!self::unique_url($article,$metname,$string)):
       ...
       $mes=$mes.'<br/>'.$text.'<code>'.$string.'</code>, ersetzt durch: ';
       $string=self::rand_string($nrand).$ext;
...
public static function rand_string($nrand) {
   #   Rueckgabe eines Zufalls-Strings aus Kleinbuchstaben.
   #   $nrand             Laenge des Zufalls-Strings
   #
   $zuf='';
   for($i=1;$i<=$nrand;$i=$i+1) $zuf=$zuf.chr(mt_rand(97,122));
   return $zuf;
   }
...
public static function get_current_clangid($req_url) {
   ...
   #   benutzte functions:
   #      self::get_current_clang($req_url,$clang_val)
   #
   $parlang=rex_config::get(REWRITER,CLANG_PARAMETER);
   $clang_val='';
   if(!empty(rex_get($parlang))) $clang_val=rex_get($parlang);
   return self::get_current_clang($req_url,$clang_val);
   }
public static function get_current_clang($req_url,$clang_val) {
   #   Rueckgabe der Sprach-Id zum aufgerufenen URL ($_SERVER['REQUEST_URI']).
   #   $req_url           aufgerufener URL ohne fuehrenden '/' und ohne Parameter
   #   $clang_val         Wert des URL-Parameters 'language' (ggf. also auch leer)
   #
   $mode   =rex_config::get(REWRITER,CLANG_MODE);
   $parlang=rex_config::get(REWRITER,CLANG_PARAMETER);
   #
...
public static function get_current_artid($req_url,$clang_id) {
   ...
   $rows=$sql->getArray($query);
   $art_id=rex_article::getNotfoundArticleId();
   if(count($rows)>0 and $rows[0]['status']>0) $art_id=$rows[0]['id'];
   return $art_id;
   }
}
